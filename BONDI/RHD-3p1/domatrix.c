...
double funct_Do(double *a, double *uu)
{
   ...
   for(i = 0; i <= 2; i++)
   {
      if(i == 0)
      {
         a[i] = -(O*sqrt(O*K*p*pow(x1,4.0)*pow(w,4.0)+((pow(K,2.0)*pow(p,2.0)+O*K*p)*pow(x1,2.0)*pow(v,2.0)+(pow(K,2.0)*pow(p,2.0)+O*K*p)*pow(x1,2.0)*pow(u,2.0)-2*O*K*p*pow(x1,2.0))*pow(w,2.0)+pow(K,2.0)*pow(p,2.0)*pow(v,4.0)+(2*pow(K,2.0)*pow(p,2.0)*pow(u,2.0)-pow(K,2.0)*pow(p,2.0)-O*K*p)*pow(v,2.0)+pow(K,2.0)*pow(p,2.0)*pow(u,4.0)+((-pow(K,2.0)*pow(p,2.0))-O*K*p)*pow(u,2.0)+O*K*p)+(O*fabs(O)-fabs(O)*K*p)*x1*w)/(fabs(O)*K*p*pow(x1,3.0)*pow(w,2.0)+fabs(O)*K*p*x1*pow(v,2.0)+fabs(O)*K*p*x1*pow(u,2.0)-O*fabs(O)*x1);
      }
      else if(i == 1)
      {
         a[i] = (O*sqrt(O*K*p*pow(x1,4.0)*pow(w,4.0)+((pow(K,2.0)*pow(p,2.0)+O*K*p)*pow(x1,2.0)*pow(v,2.0)+(pow(K,2.0)*pow(p,2.0)+O*K*p)*pow(x1,2.0)*pow(u,2.0)-2*O*K*p*pow(x1,2.0))*pow(w,2.0)+pow(K,2.0)*pow(p,2.0)*pow(v,4.0)+(2*pow(K,2.0)*pow(p,2.0)*pow(u,2.0)-pow(K,2.0)*pow(p,2.0)-O*K*p)*pow(v,2.0)+pow(K,2.0)*pow(p,2.0)*pow(u,4.0)+((-pow(K,2.0)*pow(p,2.0))-O*K*p)*pow(u,2.0)+O*K*p)+(fabs(O)*K*p-O*fabs(O))*x1*w)/(fabs(O)*K*p*pow(x1,3.0)*pow(w,2.0)+fabs(O)*K*p*x1*pow(v,2.0)+fabs(O)*K*p*x1*pow(u,2.0)-O*fabs(O)*x1);
      }
      else if(i == 2)
      {
         a[i] = w;
