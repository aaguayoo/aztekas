...
int RECONST1D(double *u, char r, lim_ *l, int *I)
{
   ...
   for(n = 0; n < eq; n++)
   {
      ...
      if(r == 'W')
      {
         l->ux1m[0*eq + n] = WENO5(u[n*(Nx1+1)+(i-3)],u[n*(Nx1+1)+(i-2)],\
         u[n*(Nx1+1)+(i-1)],u[c1(n,i)],u[n*(Nx1+1)+(i+1)]);
         l->ux1m[1*eq + n] = WENO5(u[n*(Nx1+1)+(i+2)],u[n*(Nx1+1)+(i+1)],\
         u[c1(n,i)],u[n*(Nx1+1)+(i-1)],u[n*(Nx1+1)+(i-2)]);

         l->ux1p[0*eq + n] = WENO5(u[n*(Nx1+1)+(i-2)],u[n*(Nx1+1)+(i-1)],\
         u[c1(n,i)],u[n*(Nx1+1)+(i+1)],u[n*(Nx1+1)+(i+2)]);
         l->ux1p[1*eq + n] = WENO5(u[n*(Nx1+1)+(i+3)],u[n*(Nx1+1)+(i+2)],\
         u[n*(Nx1+1)+(i+1)],u[c1(n,i)],u[n*(Nx1+1)+(i-1)]);
      }
      else if(r == 'G')
      {
...
int RECONST2D(double *u, char r, lim_ *l, int *I)
{
   ...
   for(n = 0; n < eq; n++)
   {
      ...
      if(r == 'W')
      {
         l->ux1m[0*eq + n] = WENO5(u[c2(n,i-3,j)],u[c2(n,i-2,j)],\
         u[c2(n,i-1,j)],u[c2(n,i,j)],u[c2(n,i+1,j)]);
         l->ux1m[1*eq + n] = WENO5(u[c2(n,i+2,j)],u[c2(n,i+1,j)],\
         u[c2(n,i,j)],u[c2(n,i-1,j)],u[c2(n,i-2,j)]);

         l->ux1p[0*eq + n] = WENO5(u[c2(n,i-2,j)],u[c2(n,i-1,j)],\
         u[c2(n,i,j)],u[c2(n,i+1,j)],u[c2(n,i+2,j)]);
         l->ux1p[1*eq + n] = WENO5(u[c2(n,i+3,j)],u[c2(n,i+2,j)],\
         u[c2(n,i+1,j)],u[c2(n,i,j)],u[c2(n,i-1,j)]);

         l->ux2m[0*eq + n] = WENO5(u[c2(n,i,j-3)],u[c2(n,i,j-2)],\
         u[c2(n,i,j-1)],u[c2(n,i,j)],u[c2(n,i,j+1)]);
         l->ux2m[1*eq + n] = WENO5(u[c2(n,i,j+2)],u[c2(n,i,j+1)],\
         u[c2(n,i,j)],u[c2(n,i,j-1)],u[c2(n,i,j-2)]);

         l->ux2p[0*eq + n] = WENO5(u[c2(n,i,j-2)],u[c2(n,i,j-1)],\
         u[c2(n,i,j)],u[c2(n,i,j+1)],u[c2(n,i,j+2)]);
         l->ux2p[1*eq + n] = WENO5(u[c2(n,i,j+3)],u[c2(n,i,j+2)],\
         u[c2(n,i,j+1)],u[c2(n,i,j)],u[c2(n,i,j-1)]);
      }
      else if(r == 'G')
      {
...
double MC(double A, double B)
{
   ...
   else if(A*B > 0)
   {
      ...
      else if(fabs(A) > fabs(B) && 2*fabs(B) < fabs(C))
      {
         sig = 2*B;
      }
      else if(fabs(C) < 2*fabs(A) && fabs(C) < 2*fabs(B))
      {
         sig= C;
      }
   }
